"""
JWT authentication dependency for comment_service.

The access token is issued and signed by auth_service using HS256 + SECRET_KEY.
Comment service shares the same secret so it can verify tokens locally —
no network call to auth_service required on every request.

Usage in a route:

    from comment.dependencies.auth import get_current_user, TokenPayload
