"""Lazy gRPC client for calling auth_service GetUser — used for SUPER_ADMIN checks."""
...
from admin.grpc.auth_pb2_grpc import AuthServiceStub
from admin.grpc import auth_pb2
...
async def get_user_type(target: str, user_id: str) -> str:
    """
    Return the live user type from auth_service DB.

    Returns one of: 'USER', 'COLLEGE', 'SUPER_ADMIN'.
    Falls back to 'USER' on any gRPC error so normal flow is never broken.
    """
